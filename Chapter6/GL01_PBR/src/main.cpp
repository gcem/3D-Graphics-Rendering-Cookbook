...
CameraPositioner_FirstPerson positioner(vec3(0.0f, 6.0f, 11.0f),
                                        vec3(0.0f, 4.0f, -1.0f),
                                        vec3(0.0f, 1.0f, 0.0f));
...
{
public:
    GLMeshPVP(const uint32_t *indices,
    ...
    ~GLMeshPVP() { glDeleteVertexArrays(1, &vao_); }
...
main(void)
{
    ...
    GLMeshPVP mesh(indices.data(),
                   (uint32_t)kSizeIndices,
                   (float *)vertices.data(),
    ...
    const GLuint envMaps[] = { envMap.getHandle(),

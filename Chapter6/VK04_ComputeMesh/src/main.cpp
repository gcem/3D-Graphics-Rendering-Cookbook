...
void
generateIndices(uint32_t *indices)
{
...
initMesh()
{
...

    {
        ...
        memcpy((void *)((uint8_t *)data + vertexBufferSize),
               indicesGen.data(),
               indexBufferSize);
    ...
    std::vector<const char *> shaders = {
        "data/shaders/chapter06/VK04_render.vert",
        "data/shaders/chapter06/VK04_render.frag"
    };
    std::vector<const char *> shadersColor = {
        "data/shaders/chapter06/VK04_render.vert",
        "data/shaders/chapter06/VK04_render_color.frag"
    };

    mesh = std::make_unique<ModelRenderer>(vkDev,
                                           true,
                                           meshGen->computedBuffer,
                                           meshGen->computedMemory,
                                           vertexBufferSize,
                                           indexBufferSize,
                                           imgGen->computed,
                                           imgGen->computedImageSampler,
                                           shaders,
                                           (uint32_t)sizeof(mat4),
                                           true);

    meshColor = std::make_unique<ModelRenderer>(vkDev,
                                                true,
                                                meshGen->computedBuffer,
                                                meshGen->computedMemory,
                                                vertexBufferSize,
                                                ...
                                                (uint32_t)sizeof(mat4),
                                                true,
                                                mesh->getDepthTexture(),
                                                false);
}

void
renderGUI(uint32_t imageIndex)
{
    int width, height;
...
void
updateBuffers(uint32_t imageIndex)
{
    int width, height;
    glfwGetFramebufferSize(window, &width, &height);
...
void
composeFrame(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
...
main()
{
    ...
    do {
        ...
        imgGen->fillComputeCommandBuffer(&ubo.time,
                                         sizeof(float),
                                         imgGen->computedWidth / 16,
                                         imgGen->computedHeight / 16,
                                         1);
        imgGen->submit();
        vkDeviceWaitIdle(vkDev.device);

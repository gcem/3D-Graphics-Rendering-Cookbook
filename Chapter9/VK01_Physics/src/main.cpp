...
struct MyApp : public CameraApp
{
    MyApp()
      ...
      , plane(ctx_)
      , sceneData(ctx_,
                  "data/meshes/cube.meshes",
                  "data/meshes/cube.scene",
                  "data/meshes/cube.material",
                  {},
                  {})
      , multiRenderer(ctx_,
                      sceneData,
                      "data/shaders/chapter09/VK01_Simple.vert",
                      "data/shaders/chapter09/VK01_Simple.frag")
      , imgui(ctx_)
    {
        positioner =
        ...
        for (int i = 0; i < maxCubes; i++)
            physics.addBox(vec3(1),
                           btQuaternion(0, 0, 0, 1),
                           vec3(0.f, 2.f + 3.f * i, 0.f),
                           3.f);
    }
...
generateMeshFile()
{
    ...
    Mesh cubeMesh{ .lodCount = 1,
                   .streamCount = 1,
                   .lodOffset = { 0, cubeIdxCount },
                   .streamOffset = { 0 } };

    MeshData md = { .indexData_ = std::vector<uint32_t>(cubeIdxCount, 0),
...
generateData()
{
    ...
    std::vector<MaterialDescription> materials = {
        { .albedoColor_ = gpuvec4(1, 0, 0, 1),
          .albedoMap_ = 0xFFFFFFF,
          .normalMap_ = 0xFFFFFFFF }
    };

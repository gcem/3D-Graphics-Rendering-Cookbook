...
bool
ModelRenderer::createDescriptorSet(VulkanRenderDevice &vkDev,
                                   uint32_t uniformDataSize)
{
    ...
    for (size_t i = 0; i < vkDev.swapchainImages.size(); i++) {
        ...
        const VkDescriptorImageInfo imageInfo = {
            textureSampler_,
            texture_.imageView,
            useGeneralTextureLayout_ ? VK_IMAGE_LAYOUT_GENERAL
...
void
ModelRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                                 size_t currentImage)
{
    EASY_FUNCTION();
...
void
ModelRenderer::updateUniformBuffer(VulkanRenderDevice &vkDev,
  ...
  : RendererBase(vkDev, VulkanImage())
{
    ...
    if (!createTexturedVertexBuffer(vkDev,
                                    modelFile,
                                    &storageBuffer_,

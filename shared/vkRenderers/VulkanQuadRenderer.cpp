...
void
VulkanQuadRenderer::clear()
{
    quads_.clear();
}

void
VulkanQuadRenderer::quad(float x1, float y1, float x2, float y2)
{
...
bool
VulkanQuadRenderer::createDescriptorSet(VulkanRenderDevice &vkDev)
{
    ...
    for (size_t i = 0; i < textures_.size(); i++) {
        textureDescriptors[i] =
          VkDescriptorImageInfo{ .sampler = textureSamplers_[i],
...
void
VulkanQuadRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                                      size_t currentImage)
{
    if (quads_.empty())
        return;
...
void
VulkanQuadRenderer::updateBuffer(VulkanRenderDevice &vkDev, size_t i)
{
    uploadBufferData(vkDev,
                     storageBuffersMemory_[i],
                     0,
                     quads_.data(),
                     quads_.size() * sizeof(VertexData));
}

void
VulkanQuadRenderer::pushConstants(VkCommandBuffer commandBuffer,
  ...
  , RendererBase(vkDev, VulkanImage())
{
    ...
    if (!createDepthResources(vkDev,
        ...
        !createDescriptorPool(vkDev,
                              1,
                              1,
                              1 * static_cast<uint32_t>(textures_.size()),
                              &descriptorPool_) ||
    ...
    createColorAndDepthFramebuffers(vkDev,
                                    renderPass_,
                                    VK_NULL_HANDLE /*depthTexture.imageView*/,

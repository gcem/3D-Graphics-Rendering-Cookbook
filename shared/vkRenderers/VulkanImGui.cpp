...
bool
ImGuiRenderer::createDescriptorSet(VulkanRenderDevice &vkDev)
{
    ...
    for (size_t i = 0; i < vkDev.swapchainImages.size(); i++) {
        ...
        const VkDescriptorBufferInfo bufferInfo = { uniformBuffers_[i],
                                                    0,
                                                    sizeof(mat4) };
        const VkDescriptorBufferInfo bufferInfo2 = { storageBuffer_[i],
                                                     0,
                                                     ImGuiVtxBufferSize };
        const VkDescriptorBufferInfo bufferInfo3 = { storageBuffer_[i],
                                                     ImGuiVtxBufferSize,
                                                     ImGuiIdxBufferSize };
...
bool
ImGuiRenderer::createMultiDescriptorSet(VulkanRenderDevice &vkDev)
{
    const std::array<VkDescriptorSetLayoutBinding, 4> bindings = {
        descriptorSetLayoutBinding(
            0, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT),
             ...
             VkPipelineLayout pipelineLayout)
{
    ...
    if (clipRect.x < width && clipRect.y < height && clipRect.z >= 0.0f &&
        ...
        // this is added in the Chapter 6: Using descriptor indexing in Vulkan
        // to render an ImGui UI
        if (textures.size() > 0) {
...
void
ImGuiRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                                 size_t currentImage)
{
    EASY_FUNCTION();
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        for (int cmd = 0; cmd < cmdList->CmdBuffer.Size; cmd++) {
            ...
            addImGuiItem(framebufferWidth_,
                         framebufferHeight_,
                         commandBuffer,
                         pcmd,
                         clipOff,
...
void
ImGuiRenderer::updateBuffers(VulkanRenderDevice &vkDev,
  ...
  , extTextures_(textures)
{
    ...
    if (!createColorAndDepthRenderPass(
        ...
        !createPipelineLayoutWithConstants(vkDev.device,
                                           descriptorSetLayout_,
                                           &pipelineLayout_,
                                           0,
                                           sizeof(uint32_t)) ||

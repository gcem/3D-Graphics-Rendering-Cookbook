...
struct BoundingBox
{
    ...
    {
        return 0.5f *
               vec3(max_[0] + min_[0], max_[1] + min_[1], max_[2] + min_[2]);
    }
...
inline glm::vec3
randomVec(const glm::vec3 &min, const glm::vec3 &max)
{
...
inline void
getFrustumPlanes(glm::mat4 mvp, glm::vec4 *planes)
{
    using glm::vec4;
...
inline void
getFrustumCorners(glm::mat4 mvp, glm::vec4 *points)
{
    using glm::vec4;

    const vec4 corners[] = { vec4(-1, -1, -1, 1), vec4(1, -1, -1, 1),
...
inline bool
isBoxInFrustum(glm::vec4 *frustumPlanes,
...
inline BoundingBox
combineBoxes(const std::vector<BoundingBox> &boxes)
{

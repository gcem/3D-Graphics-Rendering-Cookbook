...
int
getNumMipMapLevels2D(int w, int h)
{
    int levels = 1;
    while ((w | h) >> levels)
        levels += 1;
  ...
  : type_(type)
{
    ...
    glTextureStorage2D(handle_,
                       getNumMipMapLevels2D(width, height),
                       internalFormat,
                       width,
                       height);
}
        ...
        case GL_TEXTURE_2D: {
            ...
            if (isKTX) {
                ...
                if (!img) {
                    ...
                    if (!img) {
                        fprintf(stderr,
                                "FATAL ERROR: out of memory allocating image "
                                "for fallback texture\n");
                        exit(EXIT_FAILURE);
                    }
                }

...
{
public:
    GLMeshPVP(const std::vector<uint32_t> &indices,
...
public:
    ...
    void flush()
    {
        ...
        glNamedBufferSubData(mesh_.bufferVertices_->getHandle(),
                             0,
                             uint32_t(lines_.size() * sizeof(VertexData)),
                             lines_.data());
...
inline void
renderCameraFrustumGL(CanvasGL &canvas,
                      const mat4 &camView,
                      const mat4 &camProj,
                      const vec4 &color,
                      int numSegments = 1)

...
vec2
hammersley2d(uint32_t i, uint32_t N)
{
    return vec2(float(i) / float(N), radicalInverse_VdC(i));
}

void
convolveDiffuse(const vec3 *data,
                int srcW,
                int srcH,
                int dstW,
                int dstH,
    ...
    for (int y = 0; y != dstH; y++) {
        ...
        for (int x = 0; x != dstW; x++) {
            ...
            for (int i = 0; i != numMonteCarloSamples; i++) {
                ...
                const vec3 V2 = vec3(sin(theta2) * cos(phi2),
                                     sin(theta2) * sin(phi2),
                                     cos(theta2));
...
vec3
faceCoordsToXYZ(int i, int j, int faceID, int faceSize)
{

...
static uint32_t
shiftMeshIndices(MeshData &meshData, const std::vector<uint32_t> &meshesToMerge)
{
...
// All the meshesToMerge now have the same vertexOffset and individual index
// values are shifted by appropriate amount Here we move all the indices to
// appropriate places in the new index array
static void
mergeIndexArray(MeshData &md,
                ...
                std::map<uint32_t, uint32_t> &oldToNew)
{
    ...
    // Two offsets in the new indices array (one begins at the start, the second
    // one after all the copied indices)
...
void
mergeScene(Scene &scene, MeshData &meshData, const std::string &materialName)
{
    // Find material index
    int oldMaterial =
    ...
    for (auto i = 0u; i < scene.hierarchy_.size(); i++)
        if (scene.meshes_.contains(i) && scene.materialForNode_.contains(i) &&
            (scene.materialForNode_.at(i) == oldMaterial))
            toDelete.push_back(i);

    std::vector<uint32_t> meshesToMerge(toDelete.size());

    // Convert toDelete indices to mesh indices
    std::transform(toDelete.begin(),
                   toDelete.end(),
                   meshesToMerge.begin(),
                   [&scene](uint32_t i) { return scene.meshes_.at(i); });

    // TODO: if merged mesh transforms are non-zero, then we should
    // pre-transform individual mesh vertices in meshData using local transform
    ...
    // cutoff all but one of the merged meshes (insert the last saved mesh from
    // meshesToMerge - they are all the same)

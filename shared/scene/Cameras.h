...
struct PACKED_STRUCT LightProperties
{
    ...
    // x = light radius, for point and spot lights; x = attenuation cutoff; 2
    // flag-like floats [scattering + cast shadow]
...
static_assert(sizeof(LightProperties) == 7 * sizeof(gpuvec4),
              "Invalid sizeof(LightProperties), should be 7 * sizeof(gpuvec4)");

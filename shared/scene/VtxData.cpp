...
loadMeshData(const char *meshFile, MeshData &out)
{
    ...
    if (fread(out.meshes_.data(), sizeof(Mesh), header.meshCount, f) !=
        header.meshCount) {
        printf("Could not read mesh descriptors\n");
        exit(EXIT_FAILURE);
    }
    out.boxes_.resize(header.meshCount);
    if (fread(out.boxes_.data(), sizeof(BoundingBox), header.meshCount, f) !=
        header.meshCount) {
    ...
    if ((fread(out.indexData_.data(), 1, header.indexDataSize, f) !=
         header.indexDataSize) ||
        (fread(out.vertexData_.data(), 1, header.vertexDataSize, f) !=
...
void
saveBoundingBoxes(const char *fileName, const std::vector<BoundingBox> &boxes)
{
    FILE *f = fopen(fileName, "wb");
...
void
loadBoundingBoxes(const char *fileName, std::vector<BoundingBox> &boxes)
{
...
mergeMeshData(MeshData &m, const std::vector<MeshData *> md)
{
    ...
    for (const MeshData *i : md) {
        ...
        for (size_t j = 0; j < (uint32_t)i->meshes_.size(); j++)
            ...
            // m.vertexOffset also does not change, because vertex offsets are
            // local (i.e., baked into the indices)

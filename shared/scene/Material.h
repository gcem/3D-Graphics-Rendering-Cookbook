...
struct PACKED_STRUCT MaterialDescription final
{
    ...
    // UV anisotropic roughness (isotropic lighting models use only the first
    // value). ZW values are ignored
...
static_assert(sizeof(MaterialDescription) % 16 == 0,
              "MaterialDescription should be padded to 16 bytes");

void
saveMaterials(const char *fileName,
...
// Merge material lists from multiple scenes (follows the logic of merging in
// mergeScenes)
void
mergeMaterialLists(
    // Input:

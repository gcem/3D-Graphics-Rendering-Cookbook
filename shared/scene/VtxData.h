...
// All offsets are relative to the beginning of the data block (excluding
// headers with Mesh list)
struct Mesh final
{
    /* Number of LODs in this mesh. Strictly less than MAX_LODS, last LOD offset
     * is used as a marker only */
    uint32_t lodCount = 1;
    ...
    /* Offsets to LOD data. Last offset is used as a marker to calculate the
     * size */
    ...
    /* Information about stream element (size pretty much defines everything
     * else, the "semantics" is defined by the shader) */
    uint32_t streamElementSize[kMaxStreams] = { 0 };

    /* We could have included the streamStride[] array here to allow interleaved
       storage of attributes. For this book we assume tightly-packed
       (non-interleaved) vertex attribute streams */
...
struct MeshFileHeader
{
    ...
    /* The offset to combined mesh data (this is the base from which the offsets
     * in individual meshes start) */
...
MeshFileHeader
loadMeshData(const char *meshFile, MeshData &out);
void
saveMeshData(const char *fileName, const MeshData &m);

void
recalculateBoundingBoxes(MeshData &m);

// Combine a list of meshes to a single mesh container
MeshFileHeader
mergeMeshData(MeshData &m, const std::vector<MeshData *> md);

...
void
saveStringList(FILE *f, const std::vector<std::string> &lines);
void
loadStringList(FILE *f, std::vector<std::string> &lines);

int
addNode(Scene &scene, int parent, int level)
{
    ...
    if (parent > -1) {
        ...
        if (s == -1) {
            ...
            if (dest <= -1) {
                ...
                for (dest = s; scene.hierarchy_[dest].nextSibling_ != -1;
                     dest = scene.hierarchy_[dest].nextSibling_)
                    ;
            }
...
void
markAsChanged(Scene &scene, int node)
{
    ...
    for (int s = scene.hierarchy_[node].firstChild_; s != -1;
         s = scene.hierarchy_[s].nextSibling_)
        markAsChanged(scene, s);
}

int
findNodeByName(const Scene &scene, const std::string &name)
{
...
int
getNodeLevel(const Scene &scene, int n)
{
    int level = -1;
...
bool
mat4IsIdentity(const glm::mat4 &m);
void
fprintfMat4(FILE *f, const glm::mat4 &m);

// CPU version of global transform update []
void
recalculateGlobalTransforms(Scene &scene)
{
    ...
    for (int i = 1;
         i < MAX_NODE_LEVEL && (!scene.changedAtThisFrame_[i].empty());
         i++) {
...
void
loadMap(FILE *f, std::unordered_map<uint32_t, uint32_t> &map)
{
...
void
loadScene(const char *fileName, Scene &scene)
{
    ...
    // TODO: check > -1
    // TODO: recalculate changedAtThisLevel() - find max depth of a node [or
    // save scene.maxLevel]
...
void
saveMap(FILE *f, const std::unordered_map<uint32_t, uint32_t> &map)
{
    std::vector<uint32_t> ms;
...
void
saveScene(const char *fileName, const Scene &scene)
{
...
void
dumpTransforms(const char *fileName, const Scene &scene)
{
    ...
    for (size_t i = 0; i < scene.localTransform_.size(); i++) {
        ...
        fprintf(f,
                "Node[%d].globalDet = %f; localDet = %f\n",
                (int)i,
                glm::determinant(scene.globalTransform_[i]),
...
void
printChangedNodes(const Scene &scene)
{
    for (int i = 0;
         i < MAX_NODE_LEVEL && (!scene.changedAtThisFrame_[i].empty());
         i++) {
...
void
shiftNodes(Scene &scene, int startOffset, int nodeCount, int shiftAmount)
{
...
void
mergeMaps(ItemMap &m, const ItemMap &otherMap, int indexOffset, int itemOffset)
{
...
/**
        ...
        The simplest one is the direct "gluing" of multiple scenes into one [all
   the material lists and mesh lists are merged and indices in all scene nodes
   are shifted appropriately] The second one is creating a "grid" of objects (or
            ...
            bool mergeMaterials)
{
    ...
    // FIXME: too much logic (for all the components in a scene, though mesh
    // data and materials go separately - there are dedicated data lists)
    for (const Scene *s : scenes) {
        ...
        mergeMaps(scene.materialForNode_,
                  s->materialForNode_,
                  offs,
    ...
    // fixing 'nextSibling' fields in the old roots (zero-index in all the
    // scenes)
    offs = 1;
    int idx = 0;
    for (const Scene *s : scenes) {
    ...
    for (auto i = scene.hierarchy_.begin() + 1; i != scene.hierarchy_.end();
         i++)
        i->level_++;
}

void
dumpSceneToDot(const char *fileName, const Scene &scene, int *visited)
{
...
static void
addUniqueIdx(std::vector<uint32_t> &v, uint32_t index)
{
...
// deletion
static void
collectNodesToDelete(const Scene &scene, int node, std::vector<uint32_t> &nodes)
{
    for (int n = scene.hierarchy_[node].firstChild_; n != -1;
         n = scene.hierarchy_[n].nextSibling_) {
...
int
findLastNonDeletedItem(const Scene &scene,
...
void
shiftMapIndices(std::unordered_map<uint32_t, uint32_t> &items,
                const std::vector<int> &newIndices)
{
...
void
deleteSceneNodes(Scene &scene, const std::vector<uint32_t> &nodesToDelete)
{
    ...
    // 1.a) Move all the indicesToDelete to the end of 'nodes' array (and cut
    // them off, a variation of swap'n'pop for multiple elements)
    ...
    std::transform(scene.hierarchy_.begin(),
                   scene.hierarchy_.end(),
                   scene.hierarchy_.begin(),
                   nodeMover);

    // 3) Finally throw away the hierarchy items
    eraseSelected(scene.hierarchy_, indicesToDelete);

    // 4) As in mergeScenes() routine we also have to adjust all the
    // "components" (i.e., meshes, materials, names and transformations)

    // 4a) Transformations are stored in arrays, so we just erase the items as
    // we did with the scene.hierarchy_

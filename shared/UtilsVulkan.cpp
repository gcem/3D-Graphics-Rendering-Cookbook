...
void
CHECK(bool check, const char *fileName, int lineNumber)
{
    if (!check) {
                          ...
                          void *UserData)
{
    ...
    // performance warnings] This silences warnings like "For optimal
    // performance image layout should be VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL
    // instead of GENERAL."
...
bool
setupDebugCallbacks(VkInstance instance,
...
{
    {
        ...
        VK_CHECK(
          vkCreateDebugUtilsMessengerEXT(instance, &ci, nullptr, messenger));
    }
    {
        ...
        VK_CHECK(vkCreateDebugReportCallbackEXT(
          instance, &ci, nullptr, reportCallback));
    }
...
glslangShaderStageToVulkan(glslang_stage_t sh)
{
    switch (sh) {
...
glslang_stage_t
glslangShaderStageFromFileName(const char *fileName)
{
...
static size_t
compileShader(glslang_stage_t stage,
...
size_t
compileShaderFile(const char *file, ShaderModule &shaderModule)
{
...
createShaderModule(VkDevice device, ShaderModule *shader, const char *fileName)
{
    ...
    return vkCreateShaderModule(
      device, &createInfo, nullptr, &shader->shaderModule);
}
...
VkResult
createDeviceWithCompute(VkPhysicalDevice physicalDevice,
                        VkPhysicalDeviceFeatures deviceFeatures,
                        uint32_t graphicsFamily,
                        ...
                        VkDevice *device)
{
    ...
    if (graphicsFamily == computeFamily)
        return createDevice(
          physicalDevice, deviceFeatures, graphicsFamily, device);
...
VkResult
createDevice2(VkPhysicalDevice physicalDevice,
...
VkResult
createDevice2WithCompute(VkPhysicalDevice physicalDevice,
                         VkPhysicalDeviceFeatures2 deviceFeatures2,
                         uint32_t graphicsFamily,
                         uint32_t computeFamily,
                         VkDevice *device)
{
    ...
    if (graphicsFamily == computeFamily)
        return createDevice2(
          physicalDevice, deviceFeatures2, graphicsFamily, device);
...
VkResult
createSemaphore(VkDevice device, VkSemaphore *outSemaphore)
{
...
bool
initVulkanRenderDevice(VulkanInstance &vk,
                       ...
                       VkPhysicalDeviceFeatures deviceFeatures)
{
    ...
    VK_CHECK(
      findSuitablePhysicalDevice(vk.instance, selector, &vkDev.physicalDevice));
    vkDev.graphicsFamily =
      findQueueFamilies(vkDev.physicalDevice, VK_QUEUE_GRAPHICS_BIT);
    VK_CHECK(createDevice(vkDev.physicalDevice,
                          deviceFeatures,
                          vkDev.graphicsFamily,
                          &vkDev.device));

    vkGetDeviceQueue(
      vkDev.device, vkDev.graphicsFamily, 0, &vkDev.graphicsQueue);
    if (vkDev.graphicsQueue == nullptr)
        exit(EXIT_FAILURE);

    VkBool32 presentSupported = 0;
    vkGetPhysicalDeviceSurfaceSupportKHR(vkDev.physicalDevice,
                                         vkDev.graphicsFamily,
                                         vk.surface,
                                         &presentSupported);
    if (!presentSupported)
    ...
    VK_CHECK(createSwapchain(vkDev.device,
    ...
    VK_CHECK(
      vkCreateCommandPool(vkDev.device, &cpi, nullptr, &vkDev.commandPool));
    ...
    VK_CHECK(
      vkAllocateCommandBuffers(vkDev.device, &ai, &vkDev.commandBuffers[0]));
    return true;
}
/*
VkResult vkGetBestComputeQueue(VkPhysicalDevice physicalDevice, uint32_t*
queueFamilyIndex)
{
...
&queueFamilyPropertiesCount, queueFamilyProperties);
        ...
        for (uint32_t i = 0; i < queueFamilyPropertiesCount; i++) {
                // mask out the sparse binding bit that we aren't caring about
(yet!) and the transfer bit const VkQueueFlags maskedFlags =
(~(VK_QUEUE_TRANSFER_BIT | VK_QUEUE_SPARSE_BINDING_BIT) &
queueFamilyProperties[i].queueFlags);

                if (!(VK_QUEUE_GRAPHICS_BIT & maskedFlags) &&
(VK_QUEUE_COMPUTE_BIT & maskedFlags)) { *queueFamilyIndex = i; return
VK_SUCCESS;
                }
        }
...
*/
bool
initVulkanRenderDeviceWithCompute(VulkanInstance &vk,
                                  VulkanRenderDevice &vkDev,
                                  uint32_t width,
                                  uint32_t height,
                                  VkPhysicalDeviceFeatures deviceFeatures)
{
    ...
    VK_CHECK(findSuitablePhysicalDevice(
      vk.instance, &isDeviceSuitable, &vkDev.physicalDevice));
    vkDev.graphicsFamily =
      findQueueFamilies(vkDev.physicalDevice, VK_QUEUE_GRAPHICS_BIT);
    vkDev.computeFamily =
      findQueueFamilies(vkDev.physicalDevice, VK_QUEUE_COMPUTE_BIT);
    //	VK_CHECK(vkGetBestComputeQueue(vkDev.physicalDevice,
    //&vkDev.computeFamily));
    VK_CHECK(createDeviceWithCompute(vkDev.physicalDevice,
                                     deviceFeatures,
                                     vkDev.graphicsFamily,
                                     vkDev.computeFamily,
                                     &vkDev.device));
...

    {
        ...
        VK_CHECK(vkCreateCommandPool(
          vkDev.device, &cpi1, nullptr, &vkDev.computeCommandPool));
        ...
        VK_CHECK(vkAllocateCommandBuffers(
          vkDev.device, &ai1, &vkDev.computeCommandBuffer));
    }
...
bool
initVulkanRenderDevice2(VulkanInstance &vk,
                        VulkanRenderDevice &vkDev,
                        uint32_t width,
                        uint32_t height,
                        std::function<bool(VkPhysicalDevice)> selector,
                        VkPhysicalDeviceFeatures2 deviceFeatures2)
{
    ...
    vkDev.graphicsFamily =
      findQueueFamilies(vkDev.physicalDevice, VK_QUEUE_GRAPHICS_BIT);
    VK_CHECK(createDevice2(vkDev.physicalDevice,
                           deviceFeatures2,
                           vkDev.graphicsFamily,
                           &vkDev.device));
  ...
  bool supportScreenshots)
{
    ...
    vkDev.computeFamily =
      findQueueFamilies(vkDev.physicalDevice, VK_QUEUE_COMPUTE_BIT);
    VK_CHECK(createDevice2WithCompute(vkDev.physicalDevice,
                                      deviceFeatures2,
                                      vkDev.graphicsFamily,
                                      vkDev.computeFamily,
                                      &vkDev.device));
...
/* Combined initialization: all required rendering extensions for chapters
 * 6,7,8,9 etc. with compute queue */
bool
initVulkanRenderDevice3(VulkanInstance &vk,
                        VulkanRenderDevice &vkDev,
                        uint32_t width,
                        uint32_t height,
                        const VulkanContextFeatures &ctxFeatures)
{
    ...
    VkPhysicalDeviceFeatures deviceFeatures = {
        ...
        .geometryShader =
          (VkBool32)(ctxFeatures.geometryShader_ ? VK_TRUE : VK_FALSE),
        /* for tesselation experiments */
        .tessellationShader =
          (VkBool32)(ctxFeatures.tessellationShader_ ? VK_TRUE : VK_FALSE),
        /* for indirect instanced rendering */
        .multiDrawIndirect = VK_TRUE,
        .drawIndirectFirstInstance = VK_TRUE,
        /* for OIT and general atomic operations */
        .vertexPipelineStoresAndAtomics =
          (VkBool32)(ctxFeatures.vertexPipelineStoresAndAtomics_ ? VK_TRUE
                                                                 : VK_FALSE),
        .fragmentStoresAndAtomics =
          (VkBool32)(ctxFeatures.fragmentStoresAndAtomics_ ? VK_TRUE
                                                           : VK_FALSE),
    ...
    return initVulkanRenderDevice2WithCompute(vk,
                                              vkDev,
                                              width,
                                              height,
                                              isDeviceSuitable,
                                              deviceFeatures2,
                                              ctxFeatures.supportScreenshots_);
}
...
void
destroyVulkanInstance(VulkanInstance &vk)
{
...
bool
createTextureSampler(VkDevice device,
                     VkSampler *sampler,
                     VkFilter minFilter,
                     VkFilter maxFilter,
                     VkSamplerAddressMode addressMode)
{
    const VkSamplerCreateInfo samplerInfo = {
        ...
        .addressModeW = addressMode, // VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE
...
uint32_t
findQueueFamilies(VkPhysicalDevice device, VkQueueFlags desiredFlags)
{
...
bool
createGraphicsPipeline(VulkanRenderDevice &vkDev,
...
bool
createSharedBuffer(VulkanRenderDevice &vkDev,
                   VkDeviceSize size,
                   VkBufferUsageFlags usage,
                   VkMemoryPropertyFlags properties,
                   VkBuffer &buffer,
                   VkDeviceMemory &bufferMemory)
{
    uint32_t familyCount =
      static_cast<uint32_t>(vkDev.deviceQueueIndices.size());

    if (familyCount < 2)
        return createBuffer(vkDev.device,
                            vkDev.physicalDevice,
                            size,
                            usage,
                            properties,
    ...
    const VkBufferCreateInfo bufferInfo = {
        ...
        .size = size,
        .usage = usage,
        .sharingMode = (familyCount > 1) ? VK_SHARING_MODE_CONCURRENT
                                         : VK_SHARING_MODE_EXCLUSIVE,
        .queueFamilyIndexCount =
          static_cast<uint32_t>(vkDev.deviceQueueIndices.size()),
        .pQueueFamilyIndices =
          (familyCount > 1) ? vkDev.deviceQueueIndices.data() : nullptr
    };
    ...
    const VkMemoryAllocateInfo allocInfo = {
        ...
        .allocationSize = memRequirements.size,
        .memoryTypeIndex = findMemoryType(
          vkDev.physicalDevice, memRequirements.memoryTypeBits, properties)
    };

    VK_CHECK(
      vkAllocateMemory(vkDev.device, &allocInfo, nullptr, &bufferMemory));
...
bool
createUniformBuffer(VulkanRenderDevice &vkDev,
                    VkBuffer &buffer,
                    VkDeviceMemory &bufferMemory,
...
void
uploadBufferData(VulkanRenderDevice &vkDev,
                 ...
                 const size_t dataSize)
{
    ...
    vkMapMemory(
      vkDev.device, bufferMemory, deviceOffset, dataSize, 0, &mappedData);
    memcpy(mappedData, data, dataSize);
    vkUnmapMemory(vkDev.device, bufferMemory);
}

void
downloadBufferData(VulkanRenderDevice &vkDev,
                   const VkDeviceMemory &bufferMemory,
                   VkDeviceSize deviceOffset,
                   void *outData,
                   const size_t dataSize)
...
// createImageView(device, image, /* whatever the format is */
// VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, VkImageView*
// imageView, VK_IMAGE_VIEW_TYPE_3D, 1)
...
bool
createColorOnlyRenderPass(VulkanRenderDevice &vkDev,
                          VkRenderPass *renderPass,
                          ...
                          VkFormat colorFormat)
{
    ...
    return createColorAndDepthRenderPass(
      vkDev, false, renderPass, ci2, colorFormat);
}

bool
createColorAndDepthRenderPass(VulkanRenderDevice &vkDev,
                              bool useDepth,
                              VkRenderPass *renderPass,
                              const RenderPassCreateInfo &ci,
                              VkFormat colorFormat)
{
    ...
    VkAttachmentDescription depthAttachment = {
        .flags = 0,
        .format = useDepth ? findDepthFormat(vkDev.physicalDevice)
                           : VK_FORMAT_D32_SFLOAT,
        .samples = VK_SAMPLE_COUNT_1_BIT,
        .loadOp = offscreenInt ? VK_ATTACHMENT_LOAD_OP_LOAD
    ...
    return (
      vkCreateRenderPass(vkDev.device, &renderPassInfo, nullptr, renderPass) ==
      VK_SUCCESS);
}

bool
createDepthOnlyRenderPass(VulkanRenderDevice &vkDev,
                          VkRenderPass *renderPass,
                          const RenderPassCreateInfo &ci)
{
    VkAttachmentDescription depthAttachment = {
        ...
        .samples = VK_SAMPLE_COUNT_1_BIT,
        .loadOp = ci.clearDepth_ ? VK_ATTACHMENT_LOAD_OP_CLEAR
        ...
        .initialLayout =
          ci.clearDepth_
            ? VK_IMAGE_LAYOUT_UNDEFINED
            : (ci.flags_ & eRenderPassBit_OffscreenInternal
    ...
    if (ci.flags_ & eRenderPassBit_Offscreen) {
        ...
        /* 		dependencies.resize(2);
           ...
           //VK_PIPELINE_STAGE_DEPTH_STENCIL_ATTACHMENT_OUTPUT_BIT,
                                .srcAccessMask = VK_ACCESS_SHADER_READ_BIT,
                                .dstAccessMask =
           VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT, .dependencyFlags =
...
VkCommandBuffer
beginSingleTimeCommands(VulkanRenderDevice &vkDev)
{
...
void
endSingleTimeCommands(VulkanRenderDevice &vkDev, VkCommandBuffer commandBuffer)
{
...
void
copyBuffer(VulkanRenderDevice &vkDev,
           VkBuffer srcBuffer,
           VkBuffer dstBuffer,
           VkDeviceSize size)
{
...
void
transitionImageLayout(VulkanRenderDevice &vkDev,
                      VkImage image,
                      VkFormat format,
                      ...
                      uint32_t mipLevels)
{
    ...
    transitionImageLayoutCmd(commandBuffer,
                             image,
                             format,
                             oldLayout,
                             newLayout,
...
bool
createDepthOnlyFramebuffer(VulkanRenderDevice &vkDev,
                           uint32_t width,
                           uint32_t height,
                           VkRenderPass renderPass,
                           ...
                           VkFramebuffer *framebuffer)
{
    ...
    return (vkCreateFramebuffer(
              vkDev.device, &framebufferInfo, nullptr, framebuffer) ==
            VK_SUCCESS);
}
  ...
  std::vector<VkFramebuffer> &swapchainFramebuffers)
{
    ...
    for (size_t i = 0; i < vkDev.swapchainImages.size(); i++) {
        ...
        VK_CHECK(vkCreateFramebuffer(
          vkDev.device, &framebufferInfo, nullptr, &swapchainFramebuffers[i]));
    }
...
void
copyBufferToImage(VulkanRenderDevice &vkDev,
                  VkBuffer buffer,
                  VkImage image,
                  uint32_t width,
                  uint32_t height,
...
void
copyMIPBufferToImage(VulkanRenderDevice &vkDev,
                     VkBuffer buffer,
                     VkImage image,
                     uint32_t mipLevels,
                     uint32_t width,
...
void
copyBufferToVolume(VulkanRenderDevice &vkDev,
                   VkBuffer buffer,
                   VkImage image,
                   uint32_t width,
                   uint32_t height,
...
void
copyImageToBuffer(VulkanRenderDevice &vkDev,
                  VkImage image,
                  VkBuffer buffer,
                  uint32_t width,
                  uint32_t height,
...
void
destroyVulkanTexture(VkDevice device, VulkanTexture &texture)
{
...
void
destroyVulkanImage(VkDevice device, VulkanImage &image)
{
...
bool
updateTextureVolume(VulkanRenderDevice &vkDev,
                    ...
                    VkImageLayout sourceImageLayout)
{
    ...
    transitionImageLayout(vkDev,
                          ...
                          1);
    copyBufferToVolume(
      vkDev, stagingBuffer, textureVolume, texWidth, texHeight, texDepth);
    transitionImageLayout(vkDev,
                          textureVolume,
                          texFormat,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
bool
updateTextureImage(VulkanRenderDevice &vkDev,
...
bool
createDepthResources(VulkanRenderDevice &vkDev,
                     uint32_t width,
                     uint32_t height,
                     VulkanImage &depth)
{
    ...
    if (!createImageView(vkDev.device,
                         depth.image,
                         depthFormat,
...
bool
createPipelineLayoutWithConstants(VkDevice device,
                                  VkDescriptorSetLayout dsLayout,
                                  VkPipelineLayout *pipelineLayout,
                                  ...
                                  uint32_t fragConstSize)
{
    ...
    const VkPipelineLayoutCreateInfo pipelineLayoutInfo = {
        ...
        .pPushConstantRanges =
          (constSize == 0) ? nullptr : (vtxConstSize > 0 ? ranges : &ranges[1])
    };
...
// bool createVolume(vkDev.device, vkDev.physicalDevice, width, height, depth,
// VkFormat format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT |
// VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VkImage&
// image, VkDeviceMemory& imageMemory, VkImageCreateFlags flags)
//  createImageView(device, image, /* whatever the format is */
//  VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_ASPECT_COLOR_BIT, VkImageView*
//  imageView, VK_IMAGE_VIEW_TYPE_3D, 1)

bool
createTextureVolumeFromData(VulkanRenderDevice &vkDev,
                            VkImage &textureVolume,
                            VkDeviceMemory &textureVolumeMemory,
                            void *volumeData,
                            uint32_t texWidth,
                            ...
                            VkImageCreateFlags flags)
{
    ...
    return updateTextureVolume(vkDev,
                               textureVolume,
                               textureVolumeMemory,
                               texWidth,
                               texHeight,
...
bool
createMIPTextureImageFromData(VulkanRenderDevice &vkDev,
                              VkImage &textureImage,
                              VkDeviceMemory &textureImageMemory,
                              void *mipData,
                              uint32_t mipLevels,
                              ...
                              VkImageCreateFlags flags)
{
    ...
    transitionImageLayout(vkDev,
                          textureImage,
                          texFormat,
                          VK_IMAGE_LAYOUT_UNDEFINED /*sourceImageLayout*/,
                          VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                          layerCount,
                          mipLevels);
    copyMIPBufferToImage(vkDev,
                         stagingBuffer,
                         textureImage,
                         mipLevels,
                         texWidth,
...
bool
createTextureImageFromData(VulkanRenderDevice &vkDev,
                           VkImage &textureImage,
                           VkDeviceMemory &textureImageMemory,
                           void *imageData,
                           uint32_t texWidth,
                           ...
                           VkImageCreateFlags flags)
{
    ...
    return updateTextureImage(vkDev,
                              textureImage,
                              textureImageMemory,
                              texWidth,
                              texHeight,
...
static void
float24to32(int w, int h, const float *img24, float *img32)
{
...
bool
createMIPTextureImage(VulkanRenderDevice &vkDev,
                      const char *filename,
                      uint32_t mipLevels,
                      VkImage &textureImage,
                      VkDeviceMemory &textureImageMemory,
                      ...
                      uint32_t *height)
{
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        ...
        stbir_resize_uint8_linear(
          src, w, h, 0, dst, w / 2, h / 2, 0, (stbir_pixel_layout)texChannels);
    ...
    bool result = createMIPTextureImageFromData(vkDev,
                                                textureImage,
                                                textureImageMemory,
                                                mipData.data(),
                                                mipLevels,
...
bool
createTextureImage(VulkanRenderDevice &vkDev,
                   const char *filename,
                   VkImage &textureImage,
                   VkDeviceMemory &textureImageMemory,
                   ...
                   uint32_t *outTexHeight)
{
    ...
    bool result = createTextureImageFromData(vkDev,
                                             textureImage,
                                             textureImageMemory,
                                             pixels,
                                             texWidth,
...
size_t
allocateVertexBuffer(VulkanRenderDevice &vkDev,
...
bool
createTexturedVertexBuffer(VulkanRenderDevice &vkDev,
                           const char *filename,
                           VkBuffer *storageBuffer,
                           VkDeviceMemory *storageBufferMemory,
                           ...
                           size_t *indexBufferSize)
{
    ...
    for (unsigned i = 0; i != mesh->mNumVertices; i++) {
        ...
        vertices.push_back(
          { .pos = vec3(v.x, v.z, v.y), .tc = vec2(t.x, 1.0f - t.y) });
    }
...
bool
createPBRVertexBuffer(VulkanRenderDevice &vkDev,
                      const char *filename,
                      VkBuffer *storageBuffer,
                      VkDeviceMemory *storageBufferMemory,
                      size_t *vertexBufferSize,
                      size_t *indexBufferSize)
{
    ...
    for (unsigned i = 0; i != mesh->mNumVertices; i++) {
        ...
        vertices.push_back({ .pos = vec4(v.x, v.y, v.z, 1.0f),
...
bool
createMIPCubeTextureImage(VulkanRenderDevice &vkDev,
                          const char *filename,
                          uint32_t mipLevels,
                          VkImage &textureImage,
                          VkDeviceMemory &textureImageMemory,
...
bool
createCubeTextureImage(VulkanRenderDevice &vkDev,
                       const char *filename,
                       VkImage &textureImage,
                       VkDeviceMemory &textureImageMemory,
                       uint32_t *width,
                       uint32_t *height)
{
    ...
    return createTextureImageFromData(vkDev,
                                      textureImage,
                                      textureImageMemory,
                                      cube.data_.data(),
                                      cube.w_,
...
bool
executeComputeShader(VulkanRenderDevice &vkDev,
                     VkPipeline pipeline,
                     VkPipelineLayout pipelineLayout,
                     VkDescriptorSet ds,
                     uint32_t xsize,
                      ...
                      VkPipeline *pipeline)
{
    ...
    return vkCreateComputePipelines(
      device, 0, 1, &computePipelineCreateInfo, nullptr, pipeline);
}

/* Default DS layout for In/Out buffer pair */
bool
createComputeDescriptorSetLayout(VkDevice device,
...
void
insertComputedBufferBarrier(VulkanRenderDevice &vkDev,
                            VkCommandBuffer commandBuffer,
                            VkBuffer buffer)
{
...
void
insertComputedImageBarrier(VkCommandBuffer commandBuffer, VkImage image)
{
...
bool
createOffscreenImage(VulkanRenderDevice &vkDev,
                     VkImage &textureImage,
                     VkDeviceMemory &textureImageMemory,
                     uint32_t texWidth,
                     uint32_t texHeight,
                     ...
                     VkImageCreateFlags flags)
{
    return createImage(
      ...
      VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
        VK_IMAGE_USAGE_TRANSFER_SRC_BIT /* necessary only for screenshot */ |
...
bool
createDepthSampler(VkDevice device, VkSampler *sampler)
{
...
getMaxUsableSampleCount(VkPhysicalDevice physDevice)
{
    VkPhysicalDeviceProperties physDeviceProps;
    vkGetPhysicalDeviceProperties(physDevice, &physDeviceProps);

    VkSampleCountFlags counts =
      physDeviceProps.limits.framebufferColorSampleCounts &
      physDeviceProps.limits.framebufferDepthSampleCounts;
  ...
  , vkDev(dev)
{
    ...
    if (glfwCreateWindowSurface(
          vk.instance, (GLFWwindow *)window, nullptr, &vk.surface))
        exit(0);

    if (!initVulkanRenderDevice3(
          vk, dev, screenWidth, screenHeight, ctxFeatures))
        exit(0);
}
...
RenderPass::RenderPass(VulkanRenderDevice &vkDev,
                       bool useDepth,
                       const RenderPassCreateInfo &ci)
  : info(ci)
{
...
bool
setVkObjectName(VulkanRenderDevice &vkDev,
                void *object,
                ...
                const char *name)
{
    ...
    return (vkSetDebugUtilsObjectNameEXT(vkDev.device, &nameInfo) ==
            VK_SUCCESS);
}

void
updateTextureInDescriptorSetArray(VulkanRenderDevice &vkDev,
                                  VkDescriptorSet ds,
                                  VulkanTexture t,

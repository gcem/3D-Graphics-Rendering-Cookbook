...
/**
        For more or less abstract descriptor set setup we need to describe
   individual items ("bindings"). These are buffers, textures (samplers, but we
   call them "textures" here) and arrays of textures.
*/

/// Common information for all bindings (buffer/sampler type and shader stage(s)
...
inline TextureAttachment
makeTextureAttachment(VulkanTexture tex, VkShaderStageFlags shaderStageFlags)
{
    return TextureAttachment{
        .dInfo = { .type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
...
inline TextureAttachment
fsTextureAttachment(VulkanTexture tex)
{
    return makeTextureAttachment(tex, VK_SHADER_STAGE_FRAGMENT_BIT);
}

inline TextureArrayAttachment
fsTextureArrayAttachment(const std::vector<VulkanTexture> &textures)
{
...
inline BufferAttachment
makeBufferAttachment(VulkanBuffer buffer,
                     uint32_t offset,
                     uint32_t size,
                     VkDescriptorType type,
                     VkShaderStageFlags shaderStageFlags)
{
    return BufferAttachment{ .dInfo = { .type = type,
                                        .shaderStageFlags = shaderStageFlags },
...
inline BufferAttachment
uniformBufferAttachment(VulkanBuffer buffer,
                        uint32_t offset,
                        uint32_t size,
                        VkShaderStageFlags shaderStageFlags)
{
    return makeBufferAttachment(buffer,
                                offset,
                                size,
...
inline BufferAttachment
storageBufferAttachment(VulkanBuffer buffer,
                        uint32_t offset,
                        uint32_t size,
                        VkShaderStageFlags shaderStageFlags)
{
...
/** An aggregate structure with all the data for descriptor set (or descriptor
 * set layout) allocation */
...
/**
        The VulkanResources class keeps track of all allocated/created
   Vulkan-related objects

        The list of textures, buffers and descriptor pools.

        Individual methods from the load*() serie implement all loading

        The postprocessing chain is a "mixture" of a rendering technique,
   fullscreen postprocessing pipeline, resource manager and a frame graph

        Since RenderingTechnique, FrameGraph and PostprocessingPipeline all
   directly fill the list of rendering API commands, it is natural to generalize
   all rendering operations here.

        The "resources" part of this class is a set of methods to ease
   allocation and loading of textures and buffers. All allocated/loaded textures
   and buffers are automatically destroyed in the end.

        On top of that class we may add a (pretty simple even without
   reflection) serialization routine which reads a list of used
   buffers/textures, all the processing/rendering steps, internal parameter
   names and inter-stage dependencies.
*/
struct VulkanResources
{
    ...
    VulkanBuffer addBuffer(VkDeviceSize size,
                           VkBufferUsageFlags usage,
                           VkMemoryPropertyFlags properties,
                           bool createMapping = false);

    inline VulkanBuffer addUniformBuffer(VkDeviceSize bufferSize,
                                         bool createMapping = false)
    {
    ...
    inline VulkanBuffer addIndirectBuffer(VkDeviceSize bufferSize,
                                          bool createMapping = false)
    {
        return addBuffer(
            bufferSize,
    ...
    VulkanBuffer addVertexBuffer(uint32_t indexBufferSize,
                                 const void *indexData,
                                 uint32_t vertexBufferSize,
                                 const void *vertexData);

    VkFramebuffer addFramebuffer(RenderPass renderPass,
    ...
    VkPipeline addPipeline(VkRenderPass renderPass,
     ...
     */
    VkDescriptorPool addDescriptorPool(const DescriptorSetInfo &dsInfo,
    ...
    void updateDescriptorSet(VkDescriptorSet ds,
                             const DescriptorSetInfo &dsInfo);
...
private:
    ...
    bool createGraphicsPipeline(VulkanRenderDevice &vkDev,
                                VkRenderPass renderPass,
                                VkPipelineLayout pipelineLayout,
                                const std::vector<const char *> &shaderFiles,
                                VkPipeline *pipeline,
...
/* A helper function for inplace allocation of VulkanBuffers. Helpful to avoid
 * multiline buffer initialization in constructors */
inline VulkanBuffer
bufferAllocator(VulkanResources &resources,
                VulkanBuffer &buffer,
                VkDeviceSize size,
                bool createMapping = false)
{
...
/* Create a uniform buffer mapped to a CPU location and initialize the buffer
 * with default values */
template<class BufferT>
inline BufferAttachment
mappedUniformBufferAttachment(VulkanResources &resources,
                              BufferT **ptr,
                              VkShaderStageFlags shaderStageFlags)
{

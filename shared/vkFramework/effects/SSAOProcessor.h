...
struct SSAOProcessor : public CompositeRenderer
{
    SSAOProcessor(VulkanRenderContext &ctx,
      ...
      SSAO(ctx,
           { .buffers = { SSAOParamBuffer },
             .textures = { fsTextureAttachment(depthTex),
                           fsTextureAttachment(rotateTex) } },
           { SSAOTex },
           "data/shaders/chapter08/VK02_SSAO.frag")
      , BlurX(ctx,
              { .textures = { fsTextureAttachment(SSAOTex) } },
              { SSAOBlurXTex },
              "data/shaders/chapter08/VK02_SSAOBlurX.frag")
      , BlurY(ctx,
              { .textures = { fsTextureAttachment(SSAOBlurXTex) } },
              { SSAOBlurYTex },
              "data/shaders/chapter08/VK02_SSAOBlurY.frag")
      , SSAOFinal(ctx,
                  { .buffers = { SSAOParamBuffer },
                    .textures = { fsTextureAttachment(colorTex),
                                  fsTextureAttachment(SSAOBlurYTex) } },
                  { outputTex },
                  "data/shaders/chapter08/VK02_SSAOFinal.frag")
      ,

  ...
  , envMap_(envMap)
{
    ...
    for (const auto &f : textureFiles_) {
        auto t = asyncLoad ? ctx.resources.addSolidRGBATexture()
                           : ctx.resources.loadTexture2D(f.c_str());
...
void
VKSceneData::loadMeshes(const char *meshFile)
{
...
void
VKSceneData::loadScene(const char *sceneFile)
{
...
void
VKSceneData::updateMaterial(int matIdx)
{
    uploadBufferData(ctx.vkDev,
                     material_.memory,
                     matIdx * sizeof(MaterialDescription),
                     materials_.data() + matIdx,
                     sizeof(MaterialDescription));
}

void
VKSceneData::convertGlobalToShapeTransforms()
{
...
void
VKSceneData::recalculateAllTransforms()
{
...
void
VKSceneData::uploadGlobalTransforms()
{
    convertGlobalToShapeTransforms();
    uploadBufferData(ctx.vkDev,
                     transforms_.memory,
                     0,
                     shapeTransforms_.data(),
                     transforms_.size);
}

MultiRenderer::MultiRenderer(VulkanRenderContext &ctx,
...
void
MultiRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                                 size_t currentImage,
                                 VkFramebuffer fb,
                                 VkRenderPass rp)
{
    beginRenderPass((rp != VK_NULL_HANDLE) ? rp : renderPass_.handle,
                    (fb != VK_NULL_HANDLE) ? fb : framebuffer_,
                    commandBuffer,
                    currentImage);
    ...
    /// 0, countBuffers_[currentImage], 0, shapes.size(),
    /// sizeof(VkDrawIndirectCommand));
    /* For Vulkan 1.0 vkCmdDrawIndirect is enough */
    vkCmdDrawIndirect(commandBuffer,
                      indirect_[currentImage].buffer,
                      0,
                      (uint32_t)sceneData_.shapes_.size(),
                      sizeof(VkDrawIndirectCommand));
...
void
MultiRenderer::updateBuffers(size_t imageIndex)
{
    updateUniformBuffer((uint32_t)imageIndex, 0, sizeof(ubo_), &ubo_);
}

void
MultiRenderer::updateIndirectBuffers(size_t currentImage, bool *visibility)
{
    VkDrawIndirectCommand *data = nullptr;
    vkMapMemory(ctx_.vkDev.device,
                indirect_[currentImage].memory,
                0,
                sizeof(VkDrawIndirectCommand),
                0,
                (void **)&data);

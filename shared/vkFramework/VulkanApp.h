...
GLFWwindow *
initVulkanApp(int width, int height, Resolution *resolution = nullptr);
...
struct VulkanRenderContext
{
    ...
    VulkanRenderContext(
      ...
      depthTexture(
        resources.addDepthTexture(vkDev.framebufferWidth,
      ...
      finalRenderPass(resources.addFullScreenPass(
        true,
        RenderPassCreateInfo{ .clearColor_ = false,
                              .clearDepth_ = false,
                              .flags_ = eRenderPassBit_Last }))
      , clearRenderPass(resources.addFullScreenPass(
          true,
      ...
      swapchainFramebuffers(
        resources.addFramebuffers(screenRenderPass.handle,
                                  depthTexture.image.imageView))
      , swapchainFramebuffers_NoDepth(
          resources.addFramebuffers(screenRenderPass_NoDepth.handle))
    {
    }
    ...
    // For Chapter 8 & 9
    inline PipelineInfo pipelineParametersForOutputs(
      const std::vector<VulkanTexture> &outputs) const
    {
        return PipelineInfo{ .width = outputs.empty() ? vkDev.framebufferWidth
                                                      : outputs[0].width,
                             .height = outputs.empty() ? vkDev.framebufferHeight
                                                       : outputs[0].height,
                             .useBlending = false };
    }
                         ...
                         const VkClearValue *clearValues = nullptr)
    {
        const VkRenderPassBeginInfo renderPassInfo = {
            ...
            .renderPass = pass,
            .framebuffer =
              (fb != VK_NULL_HANDLE) ? fb : swapchainFramebuffers[currentImage],
            .renderArea = area,
...
struct VulkanApp
{
    ...
    // Check if none of the ImGui widgets were touched so our app can process
    // mouse events
...
struct CameraApp : public VulkanApp
{
    CameraApp(
      ...
      positioner(glm::vec3(0.0f, 5.0f, 10.0f),
                 vec3(0.0f, 0.0f, -1.0f),
                 vec3(0.0f, -1.0f, 0.0f))
      , camera(positioner)
    {
    ...
    virtual void update(float deltaSeconds) override
    {
        positioner.update(deltaSeconds,
                          mouseState_.pos,
                          shouldHandleMouse() ? mouseState_.pressedLeft
                                              : false);
    }
    ...
    {
        const float ratio =
          ctx_.vkDev.framebufferWidth / (float)ctx_.vkDev.framebufferHeight;

...
void
GuiRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                               size_t currentImage,
                               VkFramebuffer fb,
                               VkRenderPass rp)
{
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++) {
        ...
        for (int cmd = 0; cmd < cmdList->CmdBuffer.Size; cmd++) {
            ...
            addImGuiItem(ctx_.vkDev.framebufferWidth,
                         ctx_.vkDev.framebufferHeight,
                         commandBuffer,
                         pcmd,
                         clipOff,
                         clipScale,
...
void
GuiRenderer::updateBuffers(size_t currentImage)
{
    const ImDrawData *drawData = ImGui::GetDrawData();
...
GuiRenderer::GuiRenderer(VulkanRenderContext &ctx,
  ...
  : Renderer(ctx)
{
    ...
    DescriptorSetInfo dsInfo = {
        .buffers = { uniformBufferAttachment(VulkanBuffer{},
                                             0,
                                             sizeof(mat4),
                                             VK_SHADER_STAGE_VERTEX_BIT),
                     storageBufferAttachment(VulkanBuffer{},
                                             0,
                                             ImGuiVtxBufferSize,
                                             VK_SHADER_STAGE_VERTEX_BIT),
                     storageBufferAttachment(VulkanBuffer{},
...
void
imguiTextureWindow(const char *Title, uint32_t texId)
{
...
int
renderSceneTree(const Scene &scene, int node)
{
    int selected = -1;
    ...
    if (opened) {
        for (int ch = scene.hierarchy_[node].firstChild_; ch != -1;
             ch = scene.hierarchy_[ch].nextSibling_) {

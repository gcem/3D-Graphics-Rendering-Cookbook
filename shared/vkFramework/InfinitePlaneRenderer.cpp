...
void
InfinitePlaneRenderer::fillCommandBuffer(VkCommandBuffer commandBuffer,
                                         size_t currentImage,
                                         VkFramebuffer fb,
                                         VkRenderPass rp)
{
...
void
InfinitePlaneRenderer::updateBuffers(size_t currentImage)
{
    const UniformBuffer ubo = { proj_, view_, model_, (float)glfwGetTime() };
    uploadBufferData(
        ctx_.vkDev, uniforms_[currentImage].memory, 0, &ubo, sizeof(ubo));
}

InfinitePlaneRenderer::InfinitePlaneRenderer(
  ...
  : Renderer(ctx)
{
    ...
    initPipeline({ "data/shaders/chapter07/VK03_InfinitePlane.vert",

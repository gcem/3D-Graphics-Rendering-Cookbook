  ...
  : Renderer(ctx)
{
    ...
    initPipeline({ "data/shaders/chapter04/Lines.vert",
                   "data/shaders/chapter04/Lines.frag" },
                 pInfo);
}

void
LineCanvas::updateBuffers(size_t currentImage)
{
    if (lines_.empty())
        return;
...
void
LineCanvas::fillCommandBuffer(VkCommandBuffer commandBuffer,
                              size_t currentImage,
                              VkFramebuffer fb,
                              VkRenderPass rp)
{
...
void
LineCanvas::line(const vec3 &p1, const vec3 &p2, const vec4 &c)
{
...
void
LineCanvas::plane3d(const vec3 &o,
                    const vec3 &v1,
                    const vec3 &v2,
                    int n1,
                    int n2,
...
static void
drawBox3d_internal(LineCanvas &canvas,
                   const glm::mat4 &m,
                   const glm::vec3 &size,
                   const glm::vec4 &c)
{
...
void
renderCameraFrustum(LineCanvas &C,
                    const mat4 &camView,
                    const mat4 &camProj,
                    const vec4 &camColor)
{
    const vec3 corners[] = { vec3(+1, -1, -1), vec3(+1, -1, +1),

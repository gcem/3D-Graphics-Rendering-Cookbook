...
VulkanTexture
VulkanResources::loadCubeMap(const char *fileName, uint32_t mipLevels)
{
    ...
    if (mipLevels > 1)
        createMIPCubeTextureImage(vkDev,
                                  fileName,
                                  mipLevels,
                                  cubemap.image.image,
...
VulkanTexture
VulkanResources::loadKTX(const char *fileName)
{
    gli::texture gliTex = gli::load_ktx(fileName);
    glm::tvec3<uint32_t> extent(gliTex.extent(0));

    VulkanTexture ktx = { .width = extent.x, .height = extent.y, .depth = 4 };

    if (!createTextureImageFromData(vkDev,
                                    ktx.image.image,
                                    ktx.image.imageMemory,
    ...
    createImageView(vkDev.device,
                    ...
                    VK_IMAGE_ASPECT_COLOR_BIT,
                    &ktx.image.imageView);
    createTextureSampler(vkDev.device,
                         &ktx.sampler,
                         VK_FILTER_LINEAR,
                         VK_FILTER_LINEAR,
...
VulkanTexture
VulkanResources::loadTexture2D(const char *filename)
{
    VulkanTexture tex;
    if (!createTextureImage(vkDev,
                            filename,
                            tex.image.image,
                            tex.image.imageMemory,
                            &tex.width,
                            &tex.height)) {
...
VulkanTexture
VulkanResources::addRGBATexture(int texWidth, int texHeight, void *data)
{
    VulkanTexture tex;
    tex.width = texWidth;
    tex.height = texWidth;
...
VulkanTexture
VulkanResources::addSolidRGBATexture(uint32_t color)
{
    VulkanTexture tex;
    tex.width = 1;
    tex.height = 1;
...
VulkanTexture
VulkanResources::addColorTexture(int texWidth,
                                 int texHeight,
                                 ...
                                 VkSamplerAddressMode addressMode)
{
    ...
    VulkanTexture res = {
        .width = w, .height = h, .depth = 1, .format = colorFormat
    };

    if (!createOffscreenImage(vkDev,
                              res.image.image,
                              res.image.imageMemory,
                              w,
                              h,
                              colorFormat,
                              1,
...
VulkanTexture
VulkanResources::addDepthTexture(int texWidth,
                                 int texHeight,
                                 VkImageLayout layout)
{
    ...
    VulkanTexture depth = {
        .width = w, .height = h, .depth = 1, .format = depthFormat
    };
...
VulkanBuffer
VulkanResources::addBuffer(VkDeviceSize size,
                           VkBufferUsageFlags usage,
                           VkMemoryPropertyFlags properties,
                           bool createMapping)
{
    VulkanBuffer buffer = { .buffer = VK_NULL_HANDLE,
                            .size = 0,
...
VulkanBuffer
VulkanResources::addVertexBuffer(uint32_t indexBufferSize,
                                 const void *indexData,
                                 uint32_t vertexBufferSize,
                                 const void *vertexData)
{
    VulkanBuffer result;
    result.size = allocateVertexBuffer(vkDev,
                                       &result.buffer,
                                       &result.memory,
                                       vertexBufferSize,
                                       vertexData,
...
VkDescriptorPool
VulkanResources::addDescriptorPool(const DescriptorSetInfo &dsInfo,
                                   uint32_t dSetCount)
{
     ...
     * (int)storageBufferCount, (int)samplerCount); */
...
VkPipeline
VulkanResources::addComputePipeline(const char *shaderFile,
...
VkPipeline
VulkanResources::addPipeline(VkRenderPass renderPass,
                             VkPipelineLayout pipelineLayout,
                             const std::vector<const char *> &shaderFiles,
                             const PipelineInfo &ppInfo)
{
    ...
    if (!this->createGraphicsPipeline(vkDev,
                                      renderPass,
                                      pipelineLayout,
                                      shaderFiles,
                                      &pipeline,
                                      ...
                                      ppInfo.useDepth,
                                      ppInfo.useBlending,
                                      ppInfo.dynamicScissorState,
                                      ppInfo.width,
                                      ppInfo.height,
                                      ppInfo.patchControlPoints)) {
...
VulkanResources::addDescriptorSetLayout(const DescriptorSetInfo &dsInfo)
{
...
VkDescriptorSet
VulkanResources::addDescriptorSet(VkDescriptorPool descriptorPool,
                                  VkDescriptorSetLayout dsLayout)
{
    ...
    if (vkAllocateDescriptorSets(vkDev.device, &allocInfo, &descriptorSet) !=
        VK_SUCCESS) {
...
/*
        This routine counts all textures in all texture arrays (if any of them
   are present), creates a list of DescriptorWrite operations with required
   buffer/image info structures and calls the vkUpdateDescriptorSets()
*/
void
VulkanResources::updateDescriptorSet(VkDescriptorSet ds,
...
VkFramebuffer
VulkanResources::addFramebuffer(RenderPass renderPass,
                                const std::vector<VulkanTexture> &images)
{
    ...
    if (vkCreateFramebuffer(vkDev.device, &fbInfo, nullptr, &framebuffer) !=
        VK_SUCCESS) {
...
VulkanResources::addFramebuffers(VkRenderPass renderPass, VkImageView depthView)
{
...
RenderPass
VulkanResources::addFullScreenPass(bool useDepth,
...
RenderPass
VulkanResources::addRenderPass(const std::vector<VulkanTexture> &outputs,
                               const RenderPassCreateInfo &ci,
                               bool useDepth)
{
    VkRenderPass renderPass;
    ...
    if (outputs.size() == 1) {
        ...
        // TODO: update create...RenderPass to support multiple color
        // attachments
        if (!createColorAndDepthRenderPass(vkDev,
                                           useDepth && (outputs.size() > 1),
                                           &renderPass,
                                           ci,
                                           outputs[0].format)) {
...
RenderPass
VulkanResources::addDepthRenderPass(const std::vector<VulkanTexture> &outputs,
                                    const RenderPassCreateInfo &ci)
{
    VkRenderPass renderPass;
...
VkPipelineLayout
VulkanResources::addPipelineLayout(VkDescriptorSetLayout dsLayout,
                                   uint32_t vtxConstSize,
                                   uint32_t fragConstSize)
{
    VkPipelineLayout pipelineLayout;
    if (!createPipelineLayoutWithConstants(vkDev.device,
                                           dsLayout,
                                           &pipelineLayout,
                                           vtxConstSize,
                                           fragConstSize)) {
...
VulkanTexture
VulkanResources::createFontTexture(const char *fontFile)
{
    ...
    if (!pixels || !createTextureImageFromData(vkDev,
                                               res.image.image,
                                               res.image.imageMemory,
                                               pixels,
                                               texWidth,
...
std::pair<BufferAttachment, BufferAttachment>
VulkanResources::makeMeshBuffers(const std::vector<float> &vertices,
...
std::pair<BufferAttachment, BufferAttachment>
VulkanResources::loadMeshToBuffer(const char *filename,
                                  ...
                                  std::vector<unsigned int> &indices)
{
    ...
    if (!scene || !scene->HasMeshes()) {
        ...
        return std::pair{ BufferAttachment{ DescriptorInfo{}, nullBuffer },
                          BufferAttachment{ DescriptorInfo{}, nullBuffer } };
    }
...
std::pair<BufferAttachment, BufferAttachment>
VulkanResources::createPlaneBuffer_XZ(float sx, float sz)
{
    return makeMeshBuffers(std::vector<float>{ -sx, 0, -sz, 0, 0, 0, 1, 0,
                                               +sx, 0, -sz, 1, 0, 0, 1, 0,
                                               +sx, 0, +sz, 1, 1, 0, 1, 0,
                                               -sx, 0, +sz, 0, 1, 0, 1, 0 },
...
std::pair<BufferAttachment, BufferAttachment>
VulkanResources::createPlaneBuffer_XY(float sx, float sy)
{
    return makeMeshBuffers(std::vector<float>{ -sx, -sy, 0, 0, 0, 0, 0, 1,
                                               +sx, -sy, 0, 1, 0, 0, 0, 1,
                                               +sx, +sy, 0, 1, 1, 0, 0, 1,
                                               -sx, +sy, 0, 0, 1, 0, 0, 1 },
                           std::vector<unsigned int>{ 0u, 1u, 2u, 0u, 3u, 2u });

...
Resolution
detectResolution(int width, int height)
{
...
GLFWwindow *
initVulkanApp(int width, int height, Resolution *resolution)
{
...
void
VulkanRenderContext::updateBuffers(uint32_t imageIndex)
{
...
void
VulkanRenderContext::composeFrame(VkCommandBuffer commandBuffer,
                                  uint32_t imageIndex)
{
...
void
VulkanApp::updateBuffers(uint32_t imageIndex)
{
    ImGuiIO &io = ImGui::GetIO();
    io.DisplaySize = ImVec2((float)ctx_.vkDev.framebufferWidth,
...
void
CameraApp::handleKey(int key, bool pressed)
{
    if (key == GLFW_KEY_W)

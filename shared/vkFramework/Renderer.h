...
struct Renderer
{
    ...
    virtual void fillCommandBuffer(VkCommandBuffer cmdBuffer,
                                   ...
                                   VkRenderPass rp = VK_NULL_HANDLE) = 0;
                                ...
                                RenderPass fallbackPass = RenderPass())
    {
        ...
        {
            printf("Creating framebuffer (outputs = %d). Output0: %dx%d; "
                   "Output1: %dx%d\n",
                   (int)outputs.size(),
                   outputs[0].width,
                   outputs[0].height,
                   (outputs.size() > 1 ? outputs[1].width : 0),
                   (outputs.size() > 1 ? outputs[1].height : 0));
            fflush(stdout);
    ...
    void beginRenderPass(VkRenderPass rp,
                         VkFramebuffer fb,
                         ...
                         size_t currentImage)
    {
        ...
        const VkRect2D rect{ .offset = { 0, 0 },
                             .extent = { .width = processingWidth,
    ...
    // Updating individual textures (9 is the binding in our Chapter7-Chapter9
    // IBL scene shaders)
    void updateTexture(uint32_t textureIndex,
                       VulkanTexture newTexture,
                       uint32_t bindingIndex = 9)
    {
...
protected:
    ...
    // Descriptor set (layout + pool + sets) -> uses uniform buffers, textures,
    // framebuffers

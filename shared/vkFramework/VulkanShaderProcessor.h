...
/*
   ...
   Multiple input textures, single output [color + depth]. Possibly, can be
   extented to multiple outputs (allocate appropriate framebuffer)
*/
...
struct QuadProcessor : public VulkanShaderProcessor
{
    QuadProcessor(VulkanRenderContext &ctx,
      ...
      : VulkanShaderProcessor(
          ctx,
          ctx.pipelineParametersForOutputs(outputs),
          dsInfo,
          std::vector<const char *>{ "data/shaders/chapter08/VK02_Quad.vert",
                                     shaderFile },
          outputs,
          6 * 4,
          outputs.empty() ? ctx.screenRenderPass : RenderPass())
    {
    }
};
...
{
    BufferProcessor(VulkanRenderContext &ctx,
                    const DescriptorSetInfo &dsInfo,
                    const std::vector<VulkanTexture> &outputs,
                    const std::vector<const char *> &shaderFiles,
                    uint32_t indexBufferSize = 6 * 4,
                    RenderPass renderPass = RenderPass())
      : VulkanShaderProcessor(ctx,
                              ctx.pipelineParametersForOutputs(outputs),
                              dsInfo,
                              shaderFiles,
                              outputs,
                              indexBufferSize,
                              outputs.empty() ? ctx.screenRenderPass
                                              : renderPass)
    {
    }
};

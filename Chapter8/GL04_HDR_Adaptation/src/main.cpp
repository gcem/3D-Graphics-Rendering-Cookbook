...
CameraPositioner_FirstPerson positioner(vec3(-15.81f, 5.18f, -5.81f),
                                        vec3(0.0f, 0.0f, -1.0f),
                                        vec3(0.0f, 1.0f, 0.0f));
...
main(void)
{
    ...
    GLSceneData sceneData1("data/meshes/test.meshes",
                           "data/meshes/test.scene",
                           "data/meshes/test.materials");
    GLSceneData sceneData2("data/meshes/test2.meshes",
                           "data/meshes/test2.scene",
                           "data/meshes/test2.materials");
    ...
    GLTexture envMapIrradiance(GL_TEXTURE_CUBE_MAP,
    ...
    const GLuint pbrTextures[] = { envMap.getHandle(),
                                   envMapIrradiance.getHandle() };
    ...
    while (!glfwWindowShouldClose(app.getWindow())) {
        ...
        glClearNamedFramebufferfv(framebuffer.getHandle(),
                                  GL_COLOR,
                                  0,
        ...
        glTextureParameteri(framebuffer.getTextureColor().getHandle(),
        ...
        glNamedBufferSubData(
          perFrameDataBuffer.getHandle(), 0, sizeof(g_HDRParams), &g_HDRParams);
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha,
                            ImGui::GetStyle().Alpha * g_EnableHDR ? 1.0f
                                                                  : 0.2f);
        ImGui::Separator();
        ImGui::Text("Average luminance:");
        ImGui::Image((void *)(intptr_t)luminance1x1,
                     ImVec2(128, 128),
                     ImVec2(0.0f, 1.0f),
                     ImVec2(1.0f, 0.0f));
        ImGui::Text("Adapted luminance:");
        ImGui::Image((void *)(intptr_t)luminances[1]->getHandle(),
                     ImVec2(128, 128),
                     ImVec2(0.0f, 1.0f),
                     ImVec2(1.0f, 0.0f));
        ImGui::Separator();
        ...
        ImGui::PopStyleVar();
        ImGui::End();
        imguiTextureWindowGL("Color",
                             framebuffer.getTextureColor().getHandle());
        imguiTextureWindowGL("Luminance",
                             luminance.getTextureColor().getHandle());
        imguiTextureWindowGL("Bright Pass",
                             brightPass.getTextureColor().getHandle());

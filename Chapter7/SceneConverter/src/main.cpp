...
MaterialDescription
convertAIMaterialToDescription(const aiMaterial *M,
                               ...
                               std::vector<std::string> &opacityMaps)
{
    ...
    if (aiGetMaterialColor(M, AI_MATKEY_COLOR_TRANSPARENT, &Color) ==
        AI_SUCCESS) {
    ...
    aiTextureMapMode TextureMapMode[2] = { aiTextureMapMode_Wrap,
...
void
processLods(std::vector<uint32_t> &indices,
...
Mesh
convertAIMesh(const aiMesh *m, const SceneConfig &cfg)
{
...
void
traverse(const aiScene *sourceScene,
         Scene &scene,
         aiNode *N,
         int parent,
         int ofs)
{
    ...
    for (size_t i = 0; i < N->mNumMeshes; i++) {
        ...
        uint32_t stringID = (uint32_t)scene.names_.size();
        scene.names_.push_back(std::string(N->mName.C_Str()) + "_Mesh_" +
                               std::to_string(i));
...
void
dumpMaterial(const std::vector<std::string> &files,
             const MaterialDescription &d)
{
    printf("files: %d\n", (int)files.size());
    printf("maps: %u/%u/%u/%u/%u\n",
           (uint32_t)d.albedoMap_,
           (uint32_t)d.ambientOcclusionMap_,
           (uint32_t)d.emissiveMap_,
           (uint32_t)d.opacityMap_,
           (uint32_t)d.metallicRoughnessMap_);
    printf(" albedo:    %s\n",
...
std::string
replaceAll(const std::string &str,
           const std::string &oldSubStr,
           const std::string &newSubStr)
{
    std::string result = str;

    for (size_t p = result.find(oldSubStr); p != std::string::npos;
         p = result.find(oldSubStr))
...
std::string
lowercaseString(const std::string &s)
{
...
/* find a file in directory which "almost" coincides with the origFile (their
 * lowercase versions coincide) */
std::string
findSubstitute(const std::string &origFile)
{
    // Make absolute path
    ...
    // Iterate each file non-recursively and compare lowercase absolute path
    // with 'afile'
...
std::string
fixTextureFile(const std::string &file)
{
...
std::string
convertTexture(const std::string &file,
               ...
               const std::vector<std::string> &opacityMaps)
{
    ...
    stbi_uc *pixels = stbi_load(fixTextureFile(srcFile).c_str(),
                                &texWidth,
                                &texHeight,
                                &texChannels,
                                STBI_rgb_alpha);
...
std::vector<SceneConfig>
readConfigFile(const char *cfgFileName)
{
...
void
processScene(const SceneConfig &cfg)
{

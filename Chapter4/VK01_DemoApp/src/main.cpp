...
CameraPositioner_FirstPerson positioner_firstPerson(cameraPos,
                                                    vec3(0.0f, 0.0f, -1.0f),
                                                    vec3(0.0f, 1.0f, 0.0f));
    ...
    ImGui::Begin("Camera Control", nullptr);
    {
                                    ...
                                    // previewed before opening the combo.
        {
            for (int n = 0; n < IM_ARRAYSIZE(comboBoxItems); n++) {
                ...
                if (isSelected)
                    ImGui::SetItemDefaultFocus(); // You may set the initial
                                                  // focus when opening the
                                                  // combo (scrolling + for
                                                  // keyboard navigation
                                                  // support)
            }
...
void
composeFrame(uint32_t imageIndex, const std::vector<RendererBase *> &renderers)
{
    update3D(imageIndex);
...
bool
drawFrame(const std::vector<RendererBase *> &renderers)
{
    EASY_FUNCTION();
...
main()
{
    ...
    const std::vector<RendererBase *> renderers = {
        clear.get(),    cubeRenderer.get(), modelRenderer.get(), canvas.get(),
        canvas2d.get(), imgui.get(),        finish.get()
    };

...
/**
        ...
        For Chapter 10's demo we need to extract indirect buffer with rendering
   commands and expose it externally so that different parts of the same scene
   can be rendered (in our case - transparent and opaque objects)

        This is almost a line-by-line repetition of MultiRenderer class from
   vkFramework, but the additional 'objectIndices' parameter shows which scene
...
getOpaqueIndices(const VKSceneData &sd)
{
...
inline std::vector<int>
getTransparentIndices(const VKSceneData &sd)
{
    std::vector<int> src_indices(sd.shapes_.size());
    std::iota(src_indices.begin(), src_indices.end(), 0);
...
// Single item in the OIT buffer. See Chapter 10's GL03_OIT demo and
// "Order-independent Transparency" Recipe in the book
...
/**
        ...
        It manages lists of opaque/transparent objects and uses two
   BaseMultiRenderer instances The transparent objects renderer fills the
   auxilliary OIT linked list buffer (see VK02_Glass.frag shader) and clears
   this per-pixel transparent fragment linked list at each frame OIT buffer
   composition is also performed by this class

        Additionally, this class provides boolean flags to enable/disable
   shadows and transparent objects

        Technically, this can be implemented as a CompositeRenderer
        and OIT can be extracted to a separate class,
        but it also would require a few more barrier classes, so we opted for a
   straight-forward solution similar to a list of OpenGL commands
*/
struct FinalMultiRenderer : public Renderer
{
    ...
    inline void setLightParameters(const glm::mat4 &lightProj,
                                   const glm::mat4 &lightView)
    {
        LightParamsBuffer lightParamsBuffer = {
            .proj = lightProj,
            .view = lightView,
            .width = ctx_.vkDev.framebufferWidth,
            .height = ctx_.vkDev.framebufferHeight
        };

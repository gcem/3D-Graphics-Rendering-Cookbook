...
CameraPositioner_FirstPerson positioner(vec3(-10.0f, 3.0f, 3.0f),
                                        vec3(0.0f, 0.0f, -1.0f),
                                        vec3(0.0f, 1.0f, 0.0f));
...
main(void)
{
    ...
    GLSceneDataLazy sceneData("data/meshes/bistro_all.meshes",
                              "data/meshes/bistro_all.scene",
                              "data/meshes/bistro_all.materials");
    ...
    // create a texture view into the last mip-level (1x1 pixel) of our
    // luminance framebuffer
    GLuint luminance1x1;
    glGenTextures(1, &luminance1x1);
    glTextureView(luminance1x1,
                  GL_TEXTURE_2D,
                  luminance.getTextureColor().getHandle(),
                  GL_RGBA16F,
                  6,
                  1,
                  0,
    ...
    glTextureSubImage2D(luminance1.getHandle(),
                        0,
                        0,
                        0,
                        1,
    ...
    glNamedBufferSubData(boundingBoxesBuffer.getHandle(),
                         0,
                         reorderedBoxes.size() * sizeof(BoundingBox),
                         reorderedBoxes.data());
    ...
    auto imGuiPushFlagsAndStyles = [](bool value) {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_Alpha,
                            ImGui::GetStyle().Alpha * value ? 1.0f : 0.2f);
    };
    ...
    glTextureParameteriv(shadowMap.getTextureColor().getHandle(),
                         GL_TEXTURE_SWIZZLE_RGBA,
                         swizzleMask);
    glTextureParameteriv(shadowMap.getTextureDepth().getHandle(),
                         GL_TEXTURE_SWIZZLE_RGBA,
                         swizzleMask);
    ...
    while (!glfwWindowShouldClose(app.getWindow())) {
        ...
        glClearNamedFramebufferfv(opaqueFramebuffer.getHandle(),
                                  GL_COLOR,
                                  0,
        ...
        // cull
        {
            *numVisibleMeshesPtr = 0;
            ...
            glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                             kBufferIndex_BoundingBoxes,
                             boundingBoxesBuffer.getHandle());
            glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                             kBufferIndex_NumVisibleMeshes,
        ...
        glBindBufferBase(GL_SHADER_STORAGE_BUFFER,
                         ...
                         oitTransparencyLists.getHandle());
        ...
        if (g_EnableShadows) {
            ...
            const PerFrameData perFrameDataShadows = { .view = lightView,
                                                       .proj = lightProj };
            glNamedBufferSubData(perFrameDataBuffer.getHandle(),
                                 0,
                                 kUniformBufferSize,
        ...
        if (g_DrawTransparent) {
            ...
            mesh.draw(meshesTransparent.drawCommands_.size(),
                      &meshesTransparent);
            glFlush();
        ...
        if (g_EnableSSAO) {
            ...
            progSSAO.useProgram();
            glBindTextureUnit(0,
                              opaqueFramebuffer.getTextureDepth().getHandle());
            ...
            glBindTextureUnit(0,
                              opaqueFramebuffer.getTextureColor().getHandle());
            glBindTextureUnit(1, ssao.getTextureColor().getHandle());
            glDrawArrays(GL_TRIANGLES, 0, 6);
            framebuffer.unbind();
        } else {
            glBlitNamedFramebuffer(opaqueFramebuffer.getHandle(),
                                   framebuffer.getHandle(),
                                   0,
                                   0,
                                   width,
        ...
        if (g_EnableHDR) {
            ...
            glDrawArrays(GL_TRIANGLES, 0, 6);
            brightPass.unbind();
            glBlitNamedFramebuffer(brightPass.getHandle(),
                                   bloom2.getHandle(),
                                   0,
                                   0,
                                   256,
            ...
            glBlitNamedFramebuffer(framebuffer.getHandle(),
                                   0,
                                   0,
                                   0,
                                   width,
        ...
        ImGui::Checkbox("Show light's frustum (red) and scene AABB (white)",
                        &g_ShowLightFrustum);
        ...
        if (g_EnableShadows)
            imguiTextureWindowGL("Shadow Map",
                                 shadowMap.getTextureDepth().getHandle());
        ImGui::Render();

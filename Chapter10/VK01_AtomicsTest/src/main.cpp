...
struct AtomicRenderer : public Renderer
{
      ...
      : Renderer(ctx)
    {
        ...
        DescriptorSetInfo dsInfo = {
            .buffers = { storageBufferAttachment(VulkanBuffer{},
                                                 0,
                                                 sizeof(uint32_t),
        ...
        initPipeline({ "data/shaders/chapter10/VK01_AtomicTest.vert",
                       "data/shaders/chapter10/VK01_AtomicTest.frag" },
                     pInfo);
    }
...
{
    AnimRenderer(VulkanRenderContext &ctx,
                 std::vector<VulkanBuffer> &pointBuffers,
...
struct MyApp : public CameraApp
{
      ...
      , imgui(ctx_, std::vector<VulkanTexture>{})
    {
        ...
        {
            float w, h;
        } wh{ (float)ctx_.vkDev.framebufferWidth,

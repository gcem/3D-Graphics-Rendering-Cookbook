...
CameraPositioner_FirstPerson positioner(vec3(-31.5f, 7.5f, -9.5f),
                                        vec3(0.0f, 0.0f, -1.0f),
                                        vec3(0.0f, 1.0f, 0.0f));
...
{
public:
    GLMesh(const MeshFileHeader &header,
           const Mesh *meshes,
      ...
      , bufferIndirect_(sizeof(DrawElementsIndirectCommand) * header.meshCount +
                          sizeof(GLsizei),
                        nullptr,
                        GL_DYNAMIC_STORAGE_BIT)
    {
        ...
        drawCommands.resize(sizeof(DrawElementsIndirectCommand) * numCommands +
                            sizeof(GLsizei));
        ...
        for (uint32_t i = 0; i != numCommands; i++) {
            *cmd++ = { .count_ = meshes[i].getLODIndicesCount(0),
                       .instanceCount_ = 1,
...
main(void)
{
    ...
    const MeshFileHeader header =
      loadMeshData("data/meshes/test.meshes", meshData);

    GLMesh mesh(header,
                meshData.meshes_.data(),
                meshData.indexData_.data(),

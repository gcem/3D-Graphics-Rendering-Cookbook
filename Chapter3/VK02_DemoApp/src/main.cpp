...
fillCommandBuffers(size_t i)
{
    ...
    vkCmdBindDescriptorSets(vkDev.commandBuffers[i],
                            VK_PIPELINE_BIND_POINT_GRAPHICS,
                            ...
                            0,
                            nullptr);
    vkCmdDraw(vkDev.commandBuffers[i],
              static_cast<uint32_t>(indexBufferSize / (sizeof(unsigned int))),
              1,
              0,
              0);
...
void
updateUniformBuffer(uint32_t currentImage, const void *uboData, size_t uboSize)
{
    void *data = nullptr;
    vkMapMemory(vkDev.device,
                vkState.uniformBuffersMemory[currentImage],
                0,
                uboSize,
                0,
                &data);
...
drawOverlay()
{
    ...
    if (vkAcquireNextImageKHR(vkDev.device,
                              vkDev.swapchain,
                              0,
                              vkDev.semaphore,
                              VK_NULL_HANDLE,
